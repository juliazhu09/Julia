...
The Cox PH model is evaluated by four statistics criteria:  Akaike’s information (AIC), Schwartz’s Bayesian criterion (SBC), C-statistics, and mean absolute percentage value (MAPE). The optimal model should have low AIC, SBC, MAPE, $G^2$ value for both training and holdout dataset. In this study, the model performance on holdout dataset is considered more important than that on the training dataset.  AIC and SBC are both information criteria using likelihood value. Usually, the best model comes with lowest AIC or SBC values. AIC, SBC values are automatically generated by the models.
...
\begin{table}[htbp]
	...
	\begin{tabular}{lrr||lrr}
		\toprule
		& \multicolumn{1}{c}{Count} & \multicolumn{1}{c}{N \%}  &       & \multicolumn{1}{c}{Count} & \multicolumn{1}{c}{N \%} \\
		\midrule
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=3.5in]{reratio.png}
	\caption{Retirement rate}
	\label{fig:rerate}
\end{figure}
\subsection{Retirement model without external variables}
Extensive model selection using a variety of metrics including AIC, BIC, .... and out of sample predictive scoring was used to identify key predictive factors in the model.  Describe measures and outcome.
...
\begin{table}[htbp]
	...
	\begin{tabular}{ll|L{2.5cm}L{1.5cm}|L{2.5cm}L{1.5cm}}
		\toprule
		&       & \multicolumn{2}{c}{Period Model} & \multicolumn{2}{c}{Yearly model} \\
		\hline
...
%Need significant details about the policy.
...
\begin{table}[htbp]
	...
	\begin{tabular}{L{3cm}lllll}
		\toprule
	    \textbf{Economic Inidcator} &\multicolumn{1}{c}{\textbf{$\chi^2$}} &  \multicolumn{1}{c}{\textbf{P-value}} &  \textbf{Hazard ratio} &  \textbf{MAPE} &\multicolumn{1}{c}{\textbf{$G^2$}} \\
		\midrule
