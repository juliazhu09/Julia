...
%&=\big[\mathrm{Var}\sum{\frac{{S_0(t_j)}^{\sum\beta x)}}{{S_0(t_{j-1})}^{\sum\beta x}}\big]}\\
...
%&=\mathrm{Var}\sum{\big[{e^{h_0(t_j)}e^{exp(\sum{\beta x})}}\big]}		\\
\end{split}
\end{align*}
...
\begin{figure}[h!]
	...
	\subfloat[Retirement Prediction ]{\label{fig:repredsm}\includegraphics[width=0.35\textwidth]{repred_smooth.png}}
	\caption{Retirement Model Smoothed Baseline and Prediction Comparison}
	\label{fig:rebaselinesm}
\end{figure}
\subsubsection{Voluntary Quitting Baseline Smoothing Result}
...
\begin{figure}[h!]
	...
	\subfloat[Voluntary Quitting Prediction ]{\label{figvqepredsm}\includegraphics[width=0.35\textwidth]{vqpred_smooth.png}}
